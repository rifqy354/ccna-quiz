...
import SessionPage from '../app/study/[sessionId]/page';
import { api } from '../lib/api';
vi.mock('next/navigation',()=>({useParams:()=>({sessionId:'7'}),useRouter:()=>({push:vi.fn()})}));
vi.mock('../lib/auth-context',()=>({useAuth:()=>({token:'token'})}));
...
it('single answer replaces previous choice and submits once',async()=>{
 ...
 render(<SessionPage/>); await screen.findByText('First');
 ...
 expect(api.sessions.answer).toHaveBeenCalledWith(7,expect.objectContaining({selected_options:['B']}),'token');
});
it('shows lost session errors and a way to start again',async()=>{
 ...
 render(<SessionPage/>); expect(await screen.findByText('Session not found')).toBeTruthy();
...
it('completes exhausted sessions and displays actual score',async()=>{
 ...
 render(<SessionPage/>); expect(await screen.findByText('Session Complete!')).toBeTruthy();
 expect(api.sessions.complete).toHaveBeenCalledWith(7,'token');
});
it('supports multiple selections and allows retry after failed submission',async()=>{
 ...
 render(<SessionPage/>);await screen.findByText('First');fireEvent.click(screen.getByText('First'));fireEvent.click(screen.getByText('Second'));
 ...
 expect(await screen.findByText('Correct!')).toBeTruthy();expect(api.sessions.answer).toHaveBeenLastCalledWith(7,expect.objectContaining({selected_options:['A','B']}),'token');
});
